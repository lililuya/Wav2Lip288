...
class SyncNet(nn.Module):
    ...
    def forward(self, image, audio):
        ...
        # print("image_embedding",image_embedding.shape) # ([1, 128, 8, 8])
        # print("audio_embedding",audio_embedding.shape) # [1, 128, 8, 8])
        with open(image_path, "ab") as file:
            torch.save(image_embedding, file)
        with open(audio_path, "ab") as file:
            torch.save(audio_embedding, file)
        concat_embedding = torch.cat([image_embedding, audio_embedding], 1)
        out_score = self.merge_encoder(concat_embedding)
        # print("out_score",out_score.shape)  # out_score torch.Size([1, 1, 8, 8])
        return out_score


class SyncNetPerception_DINet(nn.Module):
    """
    use syncnet to compute perception loss
    """
    def __init__(self):
        super(SyncNetPerception_DINet, self).__init__()
